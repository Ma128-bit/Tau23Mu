...
void Plot_DMC(){
    ...
    for (int yy=0; yy<3; yy++){
        ...
        for(int k = 0; k<n; k++){
            ...
            hdata_post[k]->Scale(1./hdata_post[k]->Integral());

            hs_pre[k] = new THStack("hs_pre","");
            hs_pre[k]->Add(hmc_Bp_pre[k]);
            hs_pre[k]->Add(hmc_B0_pre[k]);
            hs_pre[k]->Add(hmc_Ds_pre[k]);
            hs_post[k] = new THStack("hs_post","");
            hs_post[k]->Add(hmc_Bp_post[k]);
            hs_post[k]->Add(hmc_B0_post[k]);
            hs_post[k]->Add(hmc_Ds_post[k]);
            
            
            //plot makeup
            double Y_max_post = std::max(hdata_post[k]->GetMaximum(), hs_post[k]->GetMaximum());
            double Y_max_pre = std::max(hdata_pre[k]->GetMaximum(), hs_pre[k]->GetMaximum());
            Y_max_pre = Y_max_pre*1.05;
            Y_max_post = Y_max_post*1.05;
            
            hdata_pre[k]->SetTitle(var[k]+" Pre_EE in "+reso_range[yy]);
            ...
            hdata_post[k]->Draw("hist");
            hs_post[k]->Draw("hist same");
            hdata_post[k]->Draw("hist same");
            
            //hs_post[k]->SetStats(0);
            TLegend*leg2 = new TLegend(x_leg_left,y_leg_left,x_leg_right,y_leg_right);
            leg2->AddEntry(hmc_Bp_post[k],"MC Bp","f");
            leg2->AddEntry(hmc_B0_post[k],"MC B0","f");
            leg2->AddEntry(hmc_Ds_post[k],"MC Ds","f");
            leg2->AddEntry(hdata_post[k],"data","f");
            leg2->Draw();
            
            c2->SaveAs("plot_MCD/"+varname+reso_range[yy]+"_postE_.png");
        }
    }
